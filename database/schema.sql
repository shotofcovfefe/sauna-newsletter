-- Supabase Schema for London Sauna News
--
-- This schema defines the database structure for storing daily scraped sauna news.
-- Execute this in your Supabase SQL editor to set up the tables and policies.

-- Create enum for news types
CREATE TYPE news_type AS ENUM ('opening', 'closure', 'major_news', 'expansion', 'other');

-- Create sauna_news table
...
CREATE INDEX idx_sauna_news_published_at ON sauna_news(published_at DESC);
CREATE INDEX idx_sauna_news_scraped_at ON sauna_news(scraped_at DESC);
CREATE INDEX idx_sauna_news_content_hash ON sauna_news(content_hash);
CREATE INDEX idx_sauna_news_news_type ON sauna_news(news_type);
CREATE INDEX idx_sauna_news_is_featured ON sauna_news(is_featured) WHERE is_featured = TRUE;
...
COMMENT ON TABLE sauna_news IS 'Stores daily scraped London sauna news from Perplexity API';
COMMENT ON COLUMN sauna_news.content_hash IS 'MD5 hash of title+summary for deduplication';
COMMENT ON COLUMN sauna_news.is_featured IS 'Manually curated or algorithmically selected top stories';
